...
async def process_endpoint(request: Request, project_id: str, process_request: ProcessRequest):
    ...
    if process_request.file_id:
        ...
        if asset_record is None:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
                    "signal": ResponceSignal.FILE_ID_ERROR.value,
                }
            )
    ...
    if len(project_files_ids) == 0:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={
                "signal": ResponceSignal.NO_FILES_ERROR.value,
            }
        )
    ...
    for asset_id, file_id in project_files_ids.items():
        ...
        file_content = process_controller.git_file_content(file_id=file_id)
        ...
        if file_chunks is None or len(file_chunks) == 0:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
                    "signal": ResponceSignal.PROCESSING_FAILED.value
                }
            )
    ...
    return JSONResponse(
        content={
            "signal": ResponceSignal.PROCESSING_SUCCESS.value,
